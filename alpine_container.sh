...
while true; do
    read -p "This will create a New Alpine LXC Container. Proceed(y/n)?" yn
    case $yn in
        [Yy]* ) break;;
        [Nn]* ) exit;;
...
pct start $CTID
pct exec $CTID -- bash -c "apk add --no-cache bash"
pct push $CTID alpine_setup.sh /alpine_setup.sh -perms 755
pct exec $CTID /alpine_setup.sh

# Get network details and show completion message
IP=$(pct exec $CTID ip a s dev eth0 | sed -n '/inet / s/\// /p' | awk '{print $2}')
info "Successfully created a Alpine LXC Container to $CTID at IP Address ${IP}"
